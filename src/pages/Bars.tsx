...
const Bars = () => {
  ...
  const filteredBars = useMemo(() => {
    let filtered = bars.filter((bar) => {
      ...
      const matchesLocation = location === "" || bar.city === location;
      const matchesCategory = category === "" || bar.category === category;
      const matchesPrice = bar.price >= priceRange[0] && bar.price <= priceRange[1] * 10;
      
      const matchesServices =
        selectedServices.length === 0 ||
        selectedServices.every((service) =>
          service === "livemusic" ? bar.hasLiveMusic :
          service === "outdoor" ? bar.hasOutdoorSeating :
  ...
  return (
    <div className="bar-pages-full">
      {/* HERO */}
      <div className="hero-full">
        <div className="hero-full-content">
          <span className="hero-badge">🍸 ETHIOPIA • NIGHTLIFE</span>
          <h1 className="hero-title">Discover Best Bars & Nightclubs</h1>
          <p className="hero-description">
            Find trendy bars, pubs, and nightlife spots across Ethiopia for unforgettable evenings
          </p>
        </div>
      </div>
      ...
      <div className="main-full">
            ...
            <div className={`filter-bar-body ${isFilterCollapsed ? "collapsed" : "expanded"}`}>
              <div className="filters-grid-full">
                <div className="filter-field">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search bars..." className="field-input" />
                </div>
                ...
                <div className="filter-field">
                  <label className="field-label"><Wine size={16} /> Bar Type</label>
                ...
                <div className="filter-field">
                  <label className="field-label"><DollarSign size={16} /> Avg Drink: ${priceRange[1] * 3}+</label>
                ...
                <div className="filter-field">
                  ...
                  <div className="amenities-group-full">
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("livemusic") ? "active" : ""}`} onClick={() => toggleService("livemusic")}>
                      <Music size={16} /> Live Music
                    </button>
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("outdoor") ? "active" : ""}`} onClick={() => toggleService("outdoor")}>
                      <Sun size={16} /> Outdoor Seating
                    </button>
        ...
        <div className="results-count-full">
          <span className="count-number">{filteredBars.length}</span> bars & nightclubs found
        </div>

        <div className={`results-container-full ${viewMode}`}>
          {filteredBars.length > 0 ? (
            filteredBars.map((bar, index) => (
              <div key={bar.id} className={`result-card-full ${viewMode}`} style={{ animationDelay: `${index * 0.05}s` }}>
                <div className="card-image-area-full">
                  <img src={bar.image} alt={bar.name} className="card-img-full" />
                  <span className="rating-badge-full"><Star size={14} /> {bar.rating}</span>
                  <span className="category-badge-full">{bar.category || "Bar"}</span>
                </div>
                <div className="card-details-full">
                  <h3 className="hotel-title-full">{bar.name}</h3>
                  <p className="hotel-location-full"><MapPin size={14} /> {bar.city}, Ethiopia</p>
                  <div className="hotel-features-full">
                    {bar.hasLiveMusic && <span className="feature-full"><Music size={12} /> Live Music</span>}
                    {bar.hasOutdoorSeating && <span className="feature-full"><Sun size={12} /> Outdoor</span>}
                  </div>
                  <p className="hotel-description-full">{bar.description?.slice(0, 90)}...</p>
                  <div className="card-footer-full">
                    <div className="price-wrapper-full">
                      <span className="price-amount-full">${bar.price}</span>
                      <span className="price-period-full">/avg drink</span>
                    </div>
                    <Link to={`/bar/${bar.id}`} className="details-link-full">View →</Link>
                  </div>
                </div>
              </div>
            ))
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-full {
          width: 100%;
          background: linear-gradient(135deg, #1a0f2a 0%, #0f081a 100%);
          background-image: url('https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=1600&h=400&fit=crop');
