...
const CakeShops = () => {
  ...
  const specialtiesList = [
    "Birthday Cakes", "Wedding Cakes", "Cupcakes", "Pastries",
    "Croissants", "Donuts", "Cookies", "Cheesecake", "Chocolate Cake",
    "Fruit Cake", "Vegan Cakes", "Gluten-Free", "Custom Cakes"
  ].sort();
  ...
  const filteredCakeShops = useMemo(() => {
    let filtered = cakeShops.filter((cake) => {
      const matchesSearch =
        cake.name.toLowerCase().includes(search.toLowerCase()) ||
        cake.city.toLowerCase().includes(search.toLowerCase()) ||
        cake.specialties.some(specialty => specialty.toLowerCase().includes(search.toLowerCase()));

      const matchesLocation = location === "" || cake.city === location;
      const matchesCategory = category === "" || cake.specialties.includes(category);
      const matchesPrice = cake.price >= priceRange[0] && cake.price <= priceRange[1] * 5;
      
      const matchesServices =
        selectedServices.length === 0 ||
        selectedServices.every((service) =>
          service === "delivery" ? cake.delivery :
  ...
  return (
    <div className="cake-pages-full">
      {/* HERO */}
      <div className="hero-full">
        <div className="hero-full-content">
          <span className="hero-badge">🎂 ETHIOPIA • SWEET TREATS</span>
          <h1 className="hero-title">Discover Best Cake Shops</h1>
          <p className="hero-description">
            Find delicious cakes, pastries, and custom desserts for every celebration across Ethiopia
          </p>
        </div>
      </div>
      ...
      <div className="main-full">
            ...
            <div className={`filter-bar-body ${isFilterCollapsed ? "collapsed" : "expanded"}`}>
              <div className="filters-grid-full">
                <div className="filter-field">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search cake shops..." className="field-input" />
                </div>
                ...
                <div className="filter-field">
                  <label className="field-label"><Cake size={16} /> Specialty</label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="field-select">
                    <option value="">All specialties</option>
                    {specialtiesList.map((specialty) => <option key={specialty} value={specialty}>{specialty}</option>)}
                  </select>
                </div>

                <div className="filter-field">
                  <label className="field-label"><DollarSign size={16} /> Price: ${priceRange[1] * 5}+</label>
                ...
                <div className="filter-field">
                  ...
                  <div className="amenities-group-full">
                    ...
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("custom") ? "active" : ""}`} onClick={() => toggleService("custom")}>
                      <Sparkles size={16} /> Custom Cakes
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div className="results-count-full">
          <span className="count-number">{filteredCakeShops.length}</span> cake shops found
        </div>

        <div className={`results-container-full ${viewMode}`}>
          {filteredCakeShops.length > 0 ? (
            filteredCakeShops.map((cake, index) => (
              <div key={cake.id} className={`result-card-full ${viewMode}`} style={{ animationDelay: `${index * 0.05}s` }}>
                <div className="card-image-area-full">
                  <img src={cake.image} alt={cake.name} className="card-img-full" />
                  <span className="rating-badge-full"><Star size={14} /> {cake.rating}</span>
                  <span className="category-badge-full">Cake Shop</span>
                  {cake.delivery && <span className="delivery-badge-full"><Truck size={12} /> Delivery</span>}
                </div>
                <div className="card-details-full">
                  <h3 className="hotel-title-full">{cake.name}</h3>
                  <p className="hotel-location-full"><MapPin size={14} /> {cake.city}, Ethiopia</p>
                  <div className="hotel-features-full">
                    {cake.specialties.slice(0, 3).map((specialty, idx) => (
                      <span key={idx} className="feature-full">{specialty}</span>
                    ))}
                  </div>
                  <p className="hotel-description-full">{cake.description?.slice(0, 90)}...</p>
                  <div className="card-footer-full">
                    <div className="price-wrapper-full">
                      <span className="price-amount-full">${cake.price}</span>
                      <span className="price-period-full">/avg</span>
                    </div>
                    <Link to={`/cake-shop/${cake.id}`} className="details-link-full">View →</Link>
                  </div>
                </div>
              </div>
            ))
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-full {
          width: 100%;
          background: linear-gradient(135deg, #2a1a20 0%, #1a1015 100%);
          background-image: url('https://images.unsplash.com/photo-1586985289688-ca3cf47d3e6e?w=1600&h=400&fit=crop');
