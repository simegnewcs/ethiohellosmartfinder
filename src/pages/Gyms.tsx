...
const Gyms = () => {
  ...
  const facilitiesList = [
    "Cardio", "Weight Training", "Pool", "Sauna", "Yoga",
    "CrossFit", "Zumba", "Spin Class", "Boxing", "Pilates",
    "Personal Training", "Group Classes", "Locker Room", "Juice Bar"
  ].sort();
  ...
  const filteredGyms = useMemo(() => {
    let filtered = gyms.filter((gym) => {
      const matchesSearch =
        ...
        gym.facilities.some(facility => facility.toLowerCase().includes(search.toLowerCase()));

      const matchesLocation = location === "" || gym.city === location;
      const matchesCategory = category === "" || gym.facilities.includes(category);
      const matchesPrice = gym.price >= priceRange[0] && gym.price <= priceRange[1] * 20;
      
      const matchesServices =
        selectedServices.length === 0 ||
        selectedServices.every((service) =>
          service === "pool" ? gym.facilities.includes("Pool") :
          service === "sauna" ? gym.facilities.includes("Sauna") :
          service === "personalTraining" ? gym.facilities.includes("Personal Training") :
          service === "24hours" ? gym.is24Hours : false
        );
  ...
  return (
    <div className="gym-pages-full">
      {/* HERO */}
      <div className="hero-full">
        <div className="hero-full-content">
          <span className="hero-badge">💪 ETHIOPIA • FITNESS</span>
          <h1 className="hero-title">Discover Best Gyms</h1>
          <p className="hero-description">
            Find top fitness centers, gyms, and workout facilities across Ethiopia
          </p>
        </div>
      </div>
      ...
      <div className="main-full">
            ...
            <div className={`filter-bar-body ${isFilterCollapsed ? "collapsed" : "expanded"}`}>
              <div className="filters-grid-full">
                <div className="filter-field">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search gyms..." className="field-input" />
                </div>
                ...
                <div className="filter-field">
                  <label className="field-label"><Dumbbell size={16} /> Facility</label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="field-select">
                    <option value="">All facilities</option>
                    {facilitiesList.map((facility) => <option key={facility} value={facility}>{facility}</option>)}
                  </select>
                </div>

                <div className="filter-field">
                  <label className="field-label"><DollarSign size={16} /> Monthly Price: ${priceRange[1] * 20}+</label>
                ...
                <div className="filter-field">
                  ...
                  <div className="amenities-group-full">
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("pool") ? "active" : ""}`} onClick={() => toggleService("pool")}>
                      <Waves size={16} /> Pool
                    </button>
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("sauna") ? "active" : ""}`} onClick={() => toggleService("sauna")}>
                      <Flame size={16} /> Sauna
                    </button>
                    <button type="button" className={`amenity-chip-full ${selectedServices.includes("personalTraining") ? "active" : ""}`} onClick={() => toggleService("personalTraining")}>
                      <Users size={16} /> Personal Training
                    </button>
        ...
        <div className="results-count-full">
          <span className="count-number">{filteredGyms.length}</span> gyms found
        </div>

        <div className={`results-container-full ${viewMode}`}>
          {filteredGyms.length > 0 ? (
            filteredGyms.map((gym, index) => (
              <div key={gym.id} className={`result-card-full ${viewMode}`} style={{ animationDelay: `${index * 0.05}s` }}>
                <div className="card-image-area-full">
                  <img src={gym.image} alt={gym.name} className="card-img-full" />
                  <span className="rating-badge-full"><Star size={14} /> {gym.rating}</span>
                  <span className="category-badge-full">Gym</span>
                  {gym.is24Hours && <span className="hours-badge-full"><Clock size={12} /> 24/7</span>}
                </div>
                <div className="card-details-full">
                  <h3 className="hotel-title-full">{gym.name}</h3>
                  <p className="hotel-location-full"><MapPin size={14} /> {gym.city}, Ethiopia</p>
                  <div className="hotel-features-full">
                    {gym.facilities.slice(0, 4).map((facility, idx) => (
                      <span key={idx} className="feature-full">{facility}</span>
                    ))}
                  </div>
                  <p className="hotel-description-full">{gym.description?.slice(0, 90)}...</p>
                  <div className="card-footer-full">
                    <div className="price-wrapper-full">
                      <span className="price-amount-full">${gym.price}</span>
                      <span className="price-period-full">/month</span>
                    </div>
                    <Link to={`/gym/${gym.id}`} className="details-link-full">View →</Link>
                  </div>
                </div>
              </div>
            ))
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-full {
          ...
          background-image: url('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=1600&h=400&fit=crop');
