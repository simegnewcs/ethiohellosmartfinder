...
const Hospitals = () => {
  ...
  const hospitalTypesList = [
    ...
    "Private Hospital", "Public Hospital", "Children's Hospital",
    "Maternity Hospital", "Psychiatric Hospital", "Rehabilitation Center"
  ].sort();
  ...
  const filteredHospitals = useMemo(() => {
    let filtered = hospitals.filter((hospital) => {
      const matchesSearch =
        ...
        hospital.description?.toLowerCase().includes(search.toLowerCase());

      const matchesLocation = location === "" || hospital.city === location;
      const matchesCategory = category === "" || hospital.category === category;
      const matchesPrice = hospital.rating >= priceRange[0] && hospital.rating <= priceRange[1];
      
      const matchesServices =
        selectedServices.length === 0 ||
        selectedServices.every((service) =>
          service === "24hours" ? hospital.is24Hours :
          service === "emergency" ? hospital.hasEmergency :
  ...
  return (
    <div className="hospital-pages-full">
      {/* HERO */}
      <div className="hero-full">
        <div className="hero-full-content">
          <span className="hero-badge">🏥 ETHIOPIA • HEALTHCARE</span>
          <h1 className="hero-title">Discover Best Hospitals</h1>
          <p className="hero-description">
            Find trusted hospitals, medical centers, and healthcare facilities across Ethiopia
          </p>
        </div>
      </div>
      ...
      <div className="main-full">
            ...
            <div className={`filter-bar-body ${isFilterCollapsed ? "collapsed" : "expanded"}`}>
              <div className="filters-grid-full">
                <div className="filter-field">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search hospitals..." className="field-input" />
                </div>
                ...
                <div className="filter-field">
                  <label className="field-label"><Hospital size={16} /> Hospital Type</label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="field-select">
                    <option value="">All types</option>
                    {hospitalTypesList.map((type) => <option key={type} value={type}>{type}</option>)}
                  </select>
                </div>
        ...
        <div className="results-count-full">
          <span className="count-number">{filteredHospitals.length}</span> hospitals found
        </div>

        <div className={`results-container-full ${viewMode}`}>
          {filteredHospitals.length > 0 ? (
            filteredHospitals.map((hospital, index) => (
              <div key={hospital.id} className={`result-card-full ${viewMode}`} style={{ animationDelay: `${index * 0.05}s` }}>
                <div className="card-image-area-full">
                  <img src={hospital.image} alt={hospital.name} className="card-img-full" />
                  <span className="rating-badge-full"><Star size={14} /> {hospital.rating}</span>
                  <span className="category-badge-full">{hospital.category || "Hospital"}</span>
                  {hospital.is24Hours && <span className="hours-badge-full"><Clock size={12} /> 24/7</span>}
                </div>
                <div className="card-details-full">
                  <h3 className="hotel-title-full">{hospital.name}</h3>
                  <p className="hotel-location-full"><MapPin size={14} /> {hospital.city}, Ethiopia</p>
                  <div className="hotel-features-full">
                    {hospital.hasEmergency && <span className="feature-full"><Ambulance size={12} /> Emergency</span>}
                    {hospital.is24Hours && <span className="feature-full"><Clock size={12} /> 24/7 Service</span>}
                  </div>
                  <p className="hotel-description-full">{hospital.description?.slice(0, 90)}...</p>
                  <div className="card-footer-full">
                    <div className="contact-info">
                      {hospital.phone && (
                        <span className="phone-number"><Phone size={12} /> {hospital.phone}</span>
                      )}
                    </div>
                    <Link to={`/hospital/${hospital.id}`} className="details-link-full">View →</Link>
                  </div>
                </div>
              </div>
            ))
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-full {
          width: 100%;
          background: linear-gradient(135deg, #1a0f0f 0%, #0f0808 100%);
          background-image: url('https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=1600&h=400&fit=crop');
